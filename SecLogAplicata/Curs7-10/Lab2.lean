...
theorem and_comm : p ∧ q → q ∧ p := by
  intros pandq
  exact And.intro (And.right pandq) (And.left pandq)
...
theorem modus_ponens_tactics': p → (p → q) → q := by
  intros hp hpq
  exact hpq hp
...
example : p ∧ (q ∨ r) → (p ∨ q) ∨ (p ∧ r) :=
  ...
  have hq: q → (p ∨ q) ∨ (p ∧ r) := by
    intros hq
    exact Or.inl $ Or.inr hq
  have hr: r → (p ∨ q) ∨ (p ∧ r) := by
    intros hr
...
example : p ∨ q → q ∨ p :=
  fun hporq =>
  have hqorp: q ∨ p := Or.elim hporq Or.inr Or.inl
  show q ∨ p from hqorp

example : p ∨ q → q ∨ p := by
  intros hporq
  exact Or.elim hporq Or.inr Or.inl
