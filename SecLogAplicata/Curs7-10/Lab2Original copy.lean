...
theorem and_comm : p ∧ q → q ∧ p :=
  fun a =>
    And.intro (And.right a) (And.left a)
...
theorem and_comm' : p ∧ q → q ∧ p := by
  intros hpandq
  exact And.intro (And.right hpandq) (And.left hpandq)
...
example : p ∧ (q ∨ r) → (p ∨ q) ∨ (p ∧ r) :=
  ...
  Or.intro_left (p ∧ r) $ Or.intro_left q hp

example : p ∧ (q ∨ r) → (p ∨ q) ∨ (p ∧ r) := by
  intros hpqr
  have hp := And.left hpqr
  have h₁: (p ∨ q) := Or.inl hp
  have h₂: (p ∨ q) ∨ (p ∧ r) := Or.inl h₁
  assumption
...
example : ¬¬p → p := by
  intros hnnp
  have fromnpFalse: ¬p → False :=
    fun hnp => absurd hnp hnnp
  exact byContradiction fromnpFalse
...
example : p ∨ q → q ∨ p :=
  fun hpq =>
  have hp: p -> q ∨ p := Or.intro_right q
  have hq: q -> q ∨ p := Or.intro_left p
  Or.elim hpq hp hq
