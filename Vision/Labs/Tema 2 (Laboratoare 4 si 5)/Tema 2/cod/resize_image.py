...
def delete_path(img, path):
    ...
    for i in range(img.shape[0]):
        c = path[i][1]
        updated_img[i,:c] = img[i, :c].copy()
        updated_img[i,c:] = img[i, c+1:].copy()
...
def decrease_height(params: Parameters, num_pixels):
    ...
    resized_img_rot = decrease_width(params, num_pixels)

    resized_img = np.rot90(resized_img_rot, k=1)
...
def resize_image(params: Parameters):
    ...
    if params.resize_option == 'micsoreazaLatime':
        ...
        resized_image = decrease_height(params, params.num_pixels_width)
        return resized_image
